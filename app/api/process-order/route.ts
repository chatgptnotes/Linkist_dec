...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (cardConfig.isFoundingMember && user && order) {
      console.log('🏆 [process-order] Processing Founders Club member...');
      ...
      try {
        ...
        console.log('✅ [process-order] User marked as founding member');
        ...
        if (cardConfig.foundersInviteCode) {
          ...
          if (codeError) {
            console.error('❌ [process-order] Error marking invite code as used:', codeError);
          } else {
            console.log('✅ [process-order] Invite code marked as used:', cardConfig.foundersInviteCode);
          }
        }
      } catch (founderError) {
        console.error('❌ [process-order] Error processing founding member status:', founderError);
